...
def parse_args() -> argparse.Namespace:
    ...
    p.add_argument('--debug-dir', type=Path, default=Path('output/column/debug_pred_mv_c2f'))
    p.add_argument('--save-ply', type=Path, default=Path('output/column/gs_ply_mv_c2f/0000_damage_opt_mv_c2f.ply'))
    p.add_argument('--save-highlight-ply', type=Path, default=Path('output/column/gs_ply_mv_c2f/0000_damage_highlight_mv_c2f.ply'))
    p.add_argument('--save-column-highlight-ply', type=Path, default=Path('output/column/gs_ply_mv_c2f/0000_column_highlight_mv_c2f.ply'))
    p.add_argument('--save-damage-only-ply', type=Path, default=Path('output/column/gs_ply_mv_c2f/0000_damage_only_mv_c2f.ply'))
    p.add_argument('--save-damage', type=Path, default=Path('output/column/gs_ply_mv_c2f/0000_damage_prob_mv_c2f.npy'))
    p.add_argument('--save-column', type=Path, default=Path('output/column/gs_ply_mv_c2f/0000_column_prob_mv_c2f.npy'))
...
def main() -> None:
    ...
    for step in range(1, args.steps + 1):
        ...
        _ignored_attn_delta, n_candidate = compute_attention_delta_for_candidates(cur_means, gaussian_feat, intr_batch, extr_batch, depth_batch, col_batch, dmg_batch, feat_maps, candidate_mask, attn, args.attn_residual_scale, args.proj_depth_rel_thresh, args.proj_depth_abs_thresh, chunk_size=args.train_attn_chunk)
        attn_delta = torch.zeros((cur_means.shape[0], 2), device=device)
    ...
    with torch.no_grad():
        ...
        _ignored_attn_delta, _ = compute_attention_delta_for_candidates(final_means, gaussian_feat, intr_t, extr_t, depth_gt, sem_col_gt, sem_spall_gt, full_feat_maps, candidate_mask, attn, args.attn_residual_scale, args.proj_depth_rel_thresh, args.proj_depth_abs_thresh, chunk_size=args.final_attn_chunk)
        attn_delta = torch.zeros((final_means.shape[0], 2), device=device)
