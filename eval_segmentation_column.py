...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser("Evaluate viewer-style strict filtered column highlights against GT masks.")
    ...
    p.add_argument("--out-dir", type=Path, default=Path("output/column/test_metrics_viewer_strict_mv_c2f_column"))

    p.add_argument("--column-thresh", type=float, default=0.97)
    ...
    p.add_argument("--column-radial-quantile", type=float, default=0.985)
...
def main() -> None:
    ...
    for dataset_dir in sorted(args.gt_root.glob("*_dataset")):
        ...
        h, w = gt_label.shape
        gt_column = (gt_label == args.damage_label_id).astype(np.uint8) if args.spall_only_supervision else (gt_label > 0).astype(np.uint8)
        ...
        pred_column = extract_added_column_mask(base_rgb, hi_rgb, args)
        m = compute_metrics(pred_column, gt_column)
        rows.append((view_name, m))

        delta_g = np.clip(hi_rgb[..., 1] - base_rgb[..., 1], 0.0, 1.0)
        delta_gr = np.clip((hi_rgb[..., 1] - hi_rgb[..., 0]) - (base_rgb[..., 1] - base_rgb[..., 0]), 0.0, 1.0)
        ...
        print(f"[{view_name}] column IoU={m.iou:.4f} P={m.precision:.4f} R={m.recall:.4f} F1={m.f1:.4f}")
    ...
    print(
        "[overall] "
        f"column IoU={overall.iou:.4f} "
        f"P={overall.precision:.4f} "
        f"R={overall.recall:.4f} "
        f"F1={overall.f1:.4f}"
    )
    print(f"[done] saved strict column viewer eval to {args.out_dir}")
